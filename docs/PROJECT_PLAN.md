...
- [x] Cut the "up to 24 hours" wait down further: the *first* search of a brand-new postal code also calls `groc/github_trigger.py` to fire the scrape workflow immediately (`workflow_dispatch`) instead of waiting for the next scheduled tick — `db.track_postal_code()`'s return value (True only the first time) prevents this from re-triggering on every repeat search of a still-empty postal code. Verified live: a real `workflow_dispatch` call actually started a run (`gh run list` confirmed it).
  - Still needs the user to add a `GH_DISPATCH_TOKEN` Vercel environment variable (a fine-grained PAT scoped to just this repo, Actions read/write) — another account-level step. Without it, this feature just no-ops and the daily schedule still covers everything.
- [x] City picker as an easier alternative to typing an exact postal code — "Or choose your city" (onboarding) and "Choose city instead" (header) open a searchable sheet of 115 major Canadian cities across every province/territory (`groc/data/canadian_cities.json`). Each city maps to one real postal code looked up directly against GeoNames' open Canadian postal code dataset (CC BY 4.0, 900,676 real entries) — never fabricated. Picking a city just fills in that postal code and submits the existing flow, so search/tracking/on-demand scraping all work unchanged. Verified with Playwright (both entry points, live filtering, selection, Escape-to-close, no mobile overflow/zoom regressions).
